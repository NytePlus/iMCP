...
actor WeChatBackend {
    ...
    private func start() throws {
        ...
        let storage = try FileManager.default.url(for: .applicationSupportDirectory, in: .userDomainMask,
        ...
        do {
            var parameters: [String: Value] = ["data_root": .string(url.path), "account": .string(account),
    ...
    func request(_ method: String, _ params: [String: Value] = [:]) throws -> Value {
        try start()
        do { return try exchange(method, params) }
        catch { if process?.isRunning != true { stop() }; throw error }
    }
    ...
    private func exchange(_ method: String, _ params: [String: Value]) throws -> Value {
        ...
        serial += 1
        let request: Value = .object(["jsonrpc": .string("2.0"), "id": .int(serial),
